...
-- LucideBlox icon pack (unchanged)
kyri.icons = {
...
function kyri.new(title, options)
    ...
    local main = make("Frame", {
        ...
        BackgroundTransparency = options.BackgroundImage and 1 or 0,  -- transparent if image is set
    ...
    if options.BackgroundImage then
        ...
        make("UICorner", {CornerRadius = UDim.new(0, 12), Parent = bgImg})

        task.spawn(function()
            local url = options.BackgroundImage
            local assetId = nil
            ...
            elseif url:match("^https?://") then
                ...
                if ok and res and res.StatusCode == 200 and res.Body and getcustomasset then
                    local fname = "kyrilib_bg/" .. url:gsub("[^%w]", "_"):sub(1, 64) .. ".png"
                    if not isfolder("kyrilib_bg") then makefolder("kyrilib_bg") end
                    writefile(fname, res.Body)
                    assetId = getcustomasset(fname)
                end
            end

            if assetId then
                bgImg.Image = assetId
                kyri.svc.tw:Create(bgImg, TweenInfo.new(0.5), {ImageTransparency = 0}):Play()
            else
                bgImg:Destroy()
